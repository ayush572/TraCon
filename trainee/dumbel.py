        ...
        with mp_pose.Pose(
            ...
            while cap.isOpened():
                ...
                if results.pose_landmarks:
                    p1 = results.pose_landmarks.landmark[11]
                    p2 = results.pose_landmarks.landmark[13]
                    p3 = results.pose_landmarks.landmark[15]
                    p4 = results.pose_landmarks.landmark[12]
                    p5 = results.pose_landmarks.landmark[14]
                    p6 = results.pose_landmarks.landmark[16]
                    if p1.visibility > 0.5 and p2.visibility > 0.5 and p3.visibility > 0.5:
                        angle = self.get_angle(p1, p2, p3)
                        if state == 'reset' and angle < 0.6:
                            count += 1
                            state = 'set'
                        if state == 'set' and angle > 1.2:
                            state = 'reset'
                        ...
                        if state == 'set' and angle > 1.2:
                            state = 'reset'
                image = cv2.putText(image, str(count), (int(cap.get(3) - 100), 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2, cv2.LINE_AA)
